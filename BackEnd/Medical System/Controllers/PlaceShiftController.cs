...
namespace Medical_System.Controllers
{
    ...
    public class PlaceShift : ControllerBase
    {
        ...
        [HttpGet("paginated")]
        public async Task<IActionResult> GetAllPlaceShiftAsync([FromQuery] string[]? filter, [FromQuery] PageFilter? pageFilter, [FromQuery] string? search = null)
        {
            var response = await _service.GetAllPlaceShiftAsync(filter, pageFilter, search);
            return Ok(response);
        }
        #endregion
    }
